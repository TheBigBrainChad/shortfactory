...
async function downloadYoutubeVideo(videoId) {
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn(cmd[0], cmd.slice(1), {
      env: { ...process.env, PATH: `${EXTRA_PATH}:${process.env.PATH}`, YTDLP_JS_RUNTIMES: 'node', YTDLP_REMOTE_COMPONENTS: 'ejs:github' },
  ...
  if (useSceneMatch && braveKey) {
    stages.push({
      ...
      fn: async () => {
        ...
        if (geminiKey) {
          appendLog(runId, '   AI vision scanning enabled — finding best 5s window in each clip');
        }
        try {
          ...
          ], { GEMINI_API_KEY: geminiKey }, (line) => appendLog(runId, line));
