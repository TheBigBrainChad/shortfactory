...
import { generateThumbnailImage, generateBackgroundQuery, pickBestBackground, generateSceneQueries } from './ai.js';
...
async function runPipelineAsync(videoId, runId, video) {
  ...
  const queriesPath = path.join(videoDir, 'scene_queries.json');
  ...
  const postTranscriptStages = [
    { name: 'subtitles', label: '✨ Creating subtitles...', script: 'subtitles.py', args: ['--transcript', transcriptPath, '--output', subtitlePath, '--style', video.caption_style || 'boxed'] },
  ...
  // Scene matching: generate queries, download clips, concatenate
  if (useSceneMatch && braveKey) {
    stages.push({
      ...
      custom: true,
      fn: async () => {
        appendLog(runId, '🎬 Generating scene search queries...');
        const transcript = JSON.parse(fs.readFileSync(transcriptPath, 'utf-8'));
        const words = transcript.words || [];

        // Segment into chunks of ~8 words
        const segments = [];
        const chunkSize = 8;
        ...
        const queries = await generateSceneQueries(video.script || video.topic, segments);
        fs.writeFileSync(queriesPath, JSON.stringify(queries, null, 2));
        appendLog(runId, `   ${queries.length} scene queries generated`);
        ...
        if (fs.existsSync(sceneBgPath)) {
          ...
          appendLog(runId, '⚠️ Scene matching failed, using fallback background');
        }
      }
    });
  }
