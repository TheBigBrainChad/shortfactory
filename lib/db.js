...
export function createVideo({ title, topic, script, voice, auto_triggered = 0 }) {
  const db = getDb();
  const id = generateId();
  db.prepare(
    `INSERT INTO videos (id, title, topic, script, voice, auto_triggered) VALUES (?, ?, ?, ?, ?, ?)`
  ).run(id, title, topic || '', script || '', voice || 'en-US-GuyNeural', auto_triggered);
  return id;
}
